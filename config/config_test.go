...
func TestShouldDraftPRRepoOverride(t *testing.T) {
	for _, tc := range []struct {
		name            string
		repoValue       string
		userValue       string
		...
		{"repo overrides user (allExceptNext over none) - closest", "allExceptNext", "none", true, false},
		{"repo overrides user (allExceptNext over none) - not closest", "allExceptNext", "none", false, true},
		{"repo overrides user (all over none) - closest", "all", "none", true, true},
		{"repo overrides user (none over all) - not closest", "none", "all", false, false},
		{"repo empty falls back to user (allExceptNext)", "", "allExceptNext", false, true},
		{"both empty -> default no draft", "", "", false, false},
	} {
		t.Run(tc.name, func(t *testing.T) {
			cfg := &Config{
				Repo: &RepoConfig{CreateDraftPRs: tc.repoValue},
				User: &UserConfig{CreateDraftPRs: tc.userValue},
			}
			actual := cfg.ShouldDraftPR(tc.isClosestToBase)
			assert.Equal(t, tc.expected, actual)
		})
